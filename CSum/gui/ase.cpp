...
class aseOutput: public wxFrame
{
    ...
    private:
        void CopyAES(wxCommandEvent& event);
        void SaveFileasHex(wxCommandEvent& event);
...
void aseFrame::Performation(wxCommandEvent& event)
{
    ...
    if(VectoredHash.size() > 0)
    {
        ...
        aseOutput* aseObject = new aseOutput("CSum | AES Output", wxTheApp->GetTopWindow());
        aseObject->Show();
    }
    else
    {
...
void aseOutput::CopyAES(wxCommandEvent& event)
{
    const size_t len = aesHash.length() + 1;
    HGLOBAL memory = GlobalAlloc(GMEM_MOVEABLE, len);
    memcpy(GlobalLock(memory), aesHash.c_str(), len);
...
void aseOutput::SaveFileasHex(wxCommandEvent& event)
{
